...
public class OutputCollitionInfo : EditorWindow
{
    ...
    public static void ShowWindow()
    {
        ...
        foreach (Transform tr in collisions)
        {
            ...
            headerTxt += "{\n";
            headerTxt += string.Format("\tD3DXVECTOR3({0}, {1}, {2}),             //平行移動\n", tr.position.x, tr.position.y, tr.position.z);
            headerTxt += string.Format("\tD3DXQUATERNION({0}, {1}, {2}, {3} ),    //回転\n", tr.rotation.x, tr.rotation.y, tr.rotation.z, tr.rotation.w);
            headerTxt += string.Format("\tD3DXVECTOR3({0}, {1}, {2} ),            //拡大\n", tr.lossyScale.x, tr.lossyScale.y, tr.lossyScale.z);
            headerTxt += "},\n";
        }
      
        StreamWriter sw = new StreamWriter("C:/Github/GameChocoBall/ChocoBall/collisionInfo.h", false, Encoding.UTF8);
        sw.Write(headerTxt);
        sw.Close();
    }
